...
class TransactionsVendorTestCase(TestCase):

    def test_resolves_list_url(self):
        resolver = self.resolve_by_name('vendor_transactions')

        self.assertEqual(resolver.func.cls, VendorViewSet)

    def test_resolves_retrieve_url(self):
        resolver = self.resolve_by_name('vendor_transaction', pk=1)
